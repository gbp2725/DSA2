...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Garrett Paradis - gbp2725@rit.edu";
...
void Application::Display(void)
{
	...
	v3CurrentPos = glm::lerp(m_stopsList[current], m_stopsList[(current + 1) % m_stopsList.size()], fTimer);

	if (fTimer >= 1.0f) //TIMER
	{
		fTimer = m_pSystem->GetDeltaTime(uClock);
		current++;
		if (current >= m_stopsList.size())
			current = 0;
	}
	//-------------------
